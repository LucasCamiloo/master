...
async function initializeApp() {
    ...
    const adPreview = getElement("adPreview");
    ...
    try {
        ...
        addSafeEventListener('productSelectionModal', 'show.bs.modal', loadProductsModal);
        addSafeEventListener('addAd', 'click', handleAddAd);
        addSafeEventListener('saveAd', 'click', () => {
            // Modal will open automatically through data-bs-toggle
        });

        // Initialize ad type buttons
...
async function handleProductSelection() {
    ...
    try {
        window.selectedProducts.forEach((product, index) => {
            const titleElement = getElement(`product${index + 1}Title`);
            const descElement = getElement(`product${index + 1}Description`);
            const priceElement = getElement(`product${index + 1}Price`);
            const imageElement = getElement(`product${index + 1}Image`);

            if (titleElement) titleElement.value = product.name;
            if (descElement) descElement.value = product.description;
            if (priceElement) priceElement.value = product.price;
            if (imageElement) imageElement.dataset.imageUrl = product.imageUrl;

            updateProductPreview(product, index + 1);
        });

        const modal = bootstrap.Modal.getInstance(getElement('productSelectionModal'));
        if (modal) {
            modal.hide();
        }
    } catch (error) {
